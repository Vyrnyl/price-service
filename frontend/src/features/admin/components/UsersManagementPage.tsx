...
type UserRole = "ADMIN" | "OFFICER" | "PUBLIC";
...
export default function UsersManagementPage() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      ...
      try {
        const fetchedUsers = await apiFetch<User[]>("/api/users", {
          method: "GET",
          credentials: "include",
        });
  ...
  const handleCreateUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const createdUser = await apiFetch<User>("/api/users", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  const handleToggleActive = async (user: User) => {
    try {
      const updatedUser = await apiFetch<User>(`/api/users/${user.id}`, {
        method: "PUT",
